...
6. implementation size gate result is recorded and does not require an unapproved split
...
8. Do not mark ready if the planning audit fails.
9. Do not implement application code in this step.
```
...
13. before marking the batch done, no required traceability row remains
    `planned` or `blocked`, and the open validation list is empty
14. before marking the batch done, final audit has passed and is recorded in PROGRESS.md and PROGRESS_STATE.md
15. AGENTS.md implementation size, validation failure, traceability, final audit, and review gates are satisfied
