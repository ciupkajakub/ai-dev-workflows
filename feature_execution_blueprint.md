...
6. include the exact feature size gate result
7. if the feature size gate requires a split, wait for user approval before writing FEATURE.md
...
11. append history rows for material changes
12. do not include secrets, credentials, private customer data, proprietary logs, or production data in FEATURE.md; use redacted examples or synthetic cases
```
...
12. if the plan exceeds the AGENTS.md implementation size gate, stop unless the larger scope is
   explicitly approved
...
7. Do not mark ready if the traceability closure table is missing required FEATURE.md items or contains unmapped required items.
8. Do not implement application code in this step.
```
...
batch status to `active` and selected task status to `in_progress`.
...
11. update WORK_INDEX.md and PRODUCT_BACKLOG.md only if lifecycle status changed
...
16. ask for explicit user approval before network access, dependency installation, destructive actions, production or staging access, credential access, GitHub mutations, browser automation in authenticated sessions, MCP/app connector side effects, or transmitting repository data to third-party services
17. AGENTS.md implementation size, validation failure, traceability, and review gates are satisfied
...
7. traceability rows touched by the task are updated to `verified`, `blocked`,
   or `accepted_gap` with evidence
8. IMPLEMENTATION.md is updated only if verified
9. WORK_INDEX.md and PRODUCT_BACKLOG.md are updated if statuses changed
10. workflow ledger mergeability is checked when workflow ledgers changed
...
13. before marking the batch done, no required traceability row remains
    `planned` or `blocked`, and the open validation list is empty
14. AGENTS.md implementation size, validation failure, traceability, and review gates are satisfied
...
PROGRESS.md and PROGRESS_STATE.md, update relevant status and traceability rows,
and stop.
...
relevant status and traceability rows, and stop.
```
